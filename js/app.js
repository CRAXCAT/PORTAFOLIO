...
function renderNav(session) {
  ...
  if (session) {
    ...
    el.innerHTML = `
      <a href="login.html" class="nav-btn ghost">Entrar</a>
...
async function loadStats() {
  try {
    const [{ data: w }, { data: f }] = await Promise.all([
      db.from('weeks').select('id'),
      db.from('week_files').select('id')
    ]);
    document.getElementById('stat-weeks').textContent = w?.length ?? '–';
  ...
  if (!weeks?.length) {
    ...
    weeks.forEach(w => {
      ...
      const thumbHtml = w.image_url
        ? `<img src="${w.image_url}" alt="" class="week-thumb-sm" onerror="this.style.display='none'"/>`
        : '';

      // File rows with eye (preview) + download buttons
      const filesHtml = files.length
        ? files.map(f => {
            const ext = f.file_name?.split('.').pop()?.toLowerCase();
            const canPreview = ['pdf','jpg','jpeg','png','gif','webp'].includes(ext);
            const previewBtn = canPreview
              ? `<a href="${f.file_url}" target="_blank" class="btn-view" title="Ver archivo">
      ...
      const bodyImgHtml = w.image_url
        ? `<img src="${w.image_url}" alt="${esc(w.title)}" class="week-img" onerror="this.style.display='none'"/>`
        : '';

      const numStr = w.number < 10 ? '0' + w.number : w.number;

      weeksHtml += `
        <div class="week-block">
          <div class="week-header" onclick="toggleWeek('${w.id}', this)">
            <div class="week-num-badge">S${numStr}</div>
            ${thumbHtml}
            <div class="week-header-info">
              <div class="week-title-h">Semana ${numStr}: ${esc(w.title)}</div>
              <div class="week-prev-h">${esc(w.description || '')}</div>
            </div>
            <div class="week-meta-right">
              <span class="week-files-badge">${files.length} archivo${files.length !== 1 ? 's' : ''}</span>
